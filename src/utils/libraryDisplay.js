...
export default async function displayLibrary(items, options = {}) {
    ...
    items.forEach((item, index) => {
        ...
        const markReceivedButton = card.querySelector('.btn-mark-received');
        if (markReceivedButton && typeof onMarkReceived === 'function' && !wishlistMode) {
          markReceivedButton.addEventListener('click', async (event) => {
            event.preventDefault();
            event.stopPropagation();
            ...
            if (typeof Swal !== 'undefined') {
              const confirmation = await Swal.fire({
                ...
                text: `"${item.Disco}" pasará a estado recibido.`,
                icon: 'question',
                showCancelButton: true,
            ...
            markReceivedButton.disabled = true;
            try {
              ...
              markReceivedButton.disabled = false;
            }
          });
        }
